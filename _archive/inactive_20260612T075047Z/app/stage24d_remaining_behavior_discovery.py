...
1) two-day compression breakout continuation
2) day-open reclaim reversal
3) Asia midpoint rejection reversal
...
REPORT_DIR = ROOT / "data" / "reports" / "stage24d_remaining_behavior_discovery"
...
ROUNDTRIP_COST_X1 = float(os.getenv("STAGE24D_ROUNDTRIP_COST_X1", "0.35"))
MAX_RUNTIME_SECONDS = int(os.getenv("STAGE24D_MAX_RUNTIME_SECONDS", "180"))
EXACT_RESERVED_SECONDS = int(os.getenv("STAGE24D_EXACT_RESERVED_SECONDS", "45"))
MAX_CANDIDATES_TOTAL = int(os.getenv("STAGE24D_MAX_CANDIDATES_TOTAL", "84"))
MAX_EXACT = int(os.getenv("STAGE24D_MAX_EXACT", "12"))
MAX_EXACT_PER_FAMILY = int(os.getenv("STAGE24D_MAX_EXACT_PER_FAMILY", "4"))
MIN_PROXY_EVENTS = int(os.getenv("STAGE24D_MIN_PROXY_EVENTS", "35"))
BOOT_SAMPLES = int(os.getenv("STAGE24D_BOOT_SAMPLES", "80"))
...
def load_data() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, str, str, str]:
    m1_override = _env_path("STAGE24D_M1_CSV")
    h1_override = _env_path("STAGE24D_H1_CSV")
    ...
    if not m1_path:
        raise FileNotFoundError("M1 CSV not found. Set STAGE24D_M1_CSV or place amarkets_xauusd_1m.csv in ~/Downloads.")
...
def candidate_grid() -> List[Candidate]:
    ...
    # Family 1: two-day compression breakout continuation.
    # Hypothesis: after two compressed days, a confirmed NY break of the two-day envelope may continue.
    for compression_atr in [0.65, 0.85]:
        for confirm_atr in [0.05, 0.15]:
            for entry_hour in [13, 15]:
                for horizon_min in [90, 180]:
                    for tp_atr, sl_atr in [(0.6, 0.65), (0.8, 0.8)]:
                        name = f"two_day_comp_break_cont_cp{compression_atr}_cf{confirm_atr}_h{horizon_min}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                        cands.append(Candidate("two_day_compression_breakout_continuation", name, dict(
    ...
    # Family 2: day-open reclaim reversal.
    # Hypothesis: intraday extension away from the daily open that reclaims the open into NY can reverse.
    for extension_atr in [0.45, 0.70]:
        for reclaim_atr in [0.00, 0.05]:
            for entry_hour in [14, 15, 16]:
                for horizon_min in [90, 180]:
                    for tp_atr, sl_atr in [(0.6, 0.65)]:
                        name = f"day_open_reclaim_rev_ext{extension_atr}_rec{reclaim_atr}_h{horizon_min}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                        cands.append(Candidate("day_open_reclaim_reversal", name, dict(
    ...
    # Family 3: Asia midpoint rejection reversal.
    # Hypothesis: a sweep of a compact Asia range that rejects through the Asia midpoint can reverse.
    for asia_range_atr_max in [0.80, 1.20]:
        for sweep_atr in [0.10, 0.20]:
            for reclaim_mid_atr in [0.00, 0.10]:
                for entry_hour in [13, 15]:
                    for horizon_min in [90, 180]:
                        for tp_atr, sl_atr in [(0.6, 0.65)]:
                            name = f"asia_mid_reject_rev_ar{asia_range_atr_max}_sw{sweep_atr}_rm{reclaim_mid_atr}_h{horizon_min}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                            cands.append(Candidate("asia_midpoint_rejection_reversal", name, dict(
                                asia_range_atr_max=asia_range_atr_max,
                                sweep_atr=sweep_atr,
                                reclaim_mid_atr=reclaim_mid_atr,
                                entry_hour=entry_hour,
                                horizon_min=horizon_min,
                                tp_atr=tp_atr,
                                sl_atr=sl_atr,
...
def generate_events_for_candidate(m15: pd.DataFrame, daily: pd.DataFrame, cand: Candidate) -> List[Event]:
    ...
    for date, day in x.groupby("date", sort=True):
        ...
        if cand.family == "two_day_compression_breakout_continuation":
            vals = [
                d.get("prev_range", np.nan), d.get("prev2_range", np.nan),
                d.get("prev_high", np.nan), d.get("prev_low", np.nan),
                d.get("prev2_high", np.nan), d.get("prev2_low", np.nan),
            ]
            if not all(np.isfinite(float(v)) for v in vals):
                continue
            prev_range, prev2_range, prev_high, prev_low, prev2_high, prev2_low = [float(v) for v in vals]
            compression_limit = float(p["compression_atr"]) * atr
            if prev_range > compression_limit or prev2_range > compression_limit:
                continue
            envelope_high = max(prev_high, prev2_high)
            envelope_low = min(prev_low, prev2_low)
            close = float(entry_bar["close"])
            confirm = float(p["confirm_atr"]) * atr
            if close >= envelope_high + confirm:
                direction = 1
            elif close <= envelope_low - confirm:
                direction = -1
            else:
                continue
            meta = {
                "prev_range": prev_range,
                "prev2_range": prev2_range,
                "envelope_high": envelope_high,
                "envelope_low": envelope_low,
                "close": close,
            }

        elif cand.family == "day_open_reclaim_reversal":
            ...
            if prior_high >= day_open + extension and close <= day_open - reclaim:
                direction = -1
            elif prior_low <= day_open - extension and close >= day_open + reclaim:
                direction = 1
            else:
                continue
            meta = {
        ...
        elif cand.family == "asia_midpoint_rejection_reversal":
            asia = day[(day["hour"] >= 0) & (day["hour"] < 7)]
            recent = day[(day["hour"] >= 7) & (day["hour"] <= entry_hour)]
            if asia.empty or recent.empty:
                continue
            asia_high = float(asia["high"].max())
            asia_low = float(asia["low"].min())
            asia_range = asia_high - asia_low
            if asia_range > float(p["asia_range_atr_max"]) * atr:
                continue
            ...
            reclaim = float(p["reclaim_mid_atr"]) * atr
            if recent_high >= asia_high + sweep and close <= mid - reclaim:
                direction = -1
            elif recent_low <= asia_low - sweep and close >= mid + reclaim:
                direction = 1
            else:
                continue
            meta = {
...
def decision_for_exact(row: Dict[str, Any]) -> str:
    if row["events"] >= 50 and row["pf_x4"] >= 1.25 and row["boot_pf_p05_x1"] >= 1.0 and row["test20_pf_x1"] >= 1.0 and row["total_x1"] > 0:
        return "STAGE24D_PROMOTION_CANDIDATE_REVIEW_ONLY"
    if row["events"] >= 50 and row["pf_x1"] >= 1.10 and row["total_x1"] > 0:
        return "STAGE24D_KEEP_WATCHLIST_ONLY"
    return "STAGE24D_REJECT"
...
def render_markdown(report: Dict[str, Any], proxy_df: pd.DataFrame, exact_df: pd.DataFrame) -> str:
    lines: List[str] = []
    lines.append("# Stage24D Remaining Behavior Discovery")
    lines.append("")
    ...
    lines.append("")
    lines.append("1. `two_day_compression_breakout_continuation` — two compressed prior days followed by envelope breakout continuation.")
    lines.append("2. `day_open_reclaim_reversal` — intraday extension away from daily open followed by reclaim/reversal.")
    lines.append("3. `asia_midpoint_rejection_reversal` — sweep of compact Asia range followed by midpoint rejection.")
    lines.append("")
    lines.append("## Counts")
    lines.append("")
    ...
    lines.append("- If no promotion appears, close Stage24D and continue with a new behavior cluster rather than widening this grid indefinitely.")
    lines.append("")
    ...
    lines.append("")
    lines.append("- `data/reports/stage24d_remaining_behavior_discovery/stage24d_remaining_behavior_discovery.json`")
    lines.append("- `data/reports/stage24d_remaining_behavior_discovery/stage24d_remaining_behavior_discovery.md`")
    lines.append("- `data/reports/stage24d_remaining_behavior_discovery/stage24d_proxy_candidates.csv`")
    lines.append("- `data/reports/stage24d_remaining_behavior_discovery/stage24d_exact_candidates.csv`")
    lines.append("- `data/reports/stage24d_remaining_behavior_discovery/stage24d_exact_trades.csv`")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    promotion_count = 0 if exact_df.empty else int((exact_df["decision"] == "STAGE24D_PROMOTION_CANDIDATE_REVIEW_ONLY").sum())
    watchlist_count = 0 if exact_df.empty else int((exact_df["decision"] == "STAGE24D_KEEP_WATCHLIST_ONLY").sum())
    if promotion_count > 0:
        decision = "STAGE24D_HAS_PROMOTION_REVIEW_CANDIDATE_RESEARCH_ONLY"
    elif watchlist_count > 0:
        decision = "STAGE24D_WATCHLIST_ONLY_KEEP_DISCOVERY_OPEN"
    else:
        decision = "STAGE24D_NO_PROMOTION_KEEP_DISCOVERY_OPEN"
    ...
    proxy_df.to_csv(REPORT_DIR / "stage24d_proxy_candidates.csv", index=False)
    exact_df.to_csv(REPORT_DIR / "stage24d_exact_candidates.csv", index=False)
    trades_df.to_csv(REPORT_DIR / "stage24d_exact_trades.csv", index=False)
    (REPORT_DIR / "stage24d_remaining_behavior_discovery.json").write_text(json.dumps(json_safe(report), indent=2), encoding="utf-8")
    (REPORT_DIR / "stage24d_remaining_behavior_discovery.md").write_text(render_markdown(report, proxy_df, exact_df), encoding="utf-8")
    print(f"Decision: {decision}")
    print(f"Report: {REPORT_DIR / 'stage24d_remaining_behavior_discovery.md'}")
