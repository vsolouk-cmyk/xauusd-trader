...
STAGE_NAME = "stage23a_lite_proxy_exact_discovery"
...
def decide_exact(summary: Dict[str, Any]) -> str:
    ...
    if events >= MIN_EXACT_EVENTS and pf_x1 >= 1.25 and pf_x4 >= 1.05 and test20 >= 1.05 and boot >= 1.0 and median > 0 and y_ok:
        return "STAGE23_PROMOTION_CANDIDATE_REVIEW_ONLY"
    if events >= MIN_EXACT_EVENTS and pf_x1 >= 1.15 and pf_x4 >= 1.0 and test20 >= 0.95 and boot >= 0.85 and y_ok:
        return "STAGE23_KEEP_WATCHLIST_ONLY"
    return "STAGE23_REJECT"
...
def events_london_oneway_fade(m15: pd.DataFrame, params: Dict[str, Any]) -> pd.DataFrame:
    rows = []
    for _, g in _day_groups(m15):
        t = pd.to_datetime(g["time"])
        london = g[(t.dt.hour >= 7) & (t.dt.hour < 13)]
        if len(london) < 8:
            continue
        ...
        continuation = (entry_close - london_close) * (1 if move > 0 else -1) / atr
        if move_atr < float(params["london_move_atr_min"]):
            continue
        if efficiency < float(params["eff_min"]):
            continue
        if continuation < float(params["ny_cont_atr_min"]):
            continue
        rows.append(
            {
                ...
                "continuation_atr": continuation,
            }
        )
    return pd.DataFrame(rows)


def events_asia_london_double_expansion_reversal(m15: pd.DataFrame, params: Dict[str, Any]) -> pd.DataFrame:
    rows = []
    for _, g in _day_groups(m15):
        t = pd.to_datetime(g["time"])
        asia = g[(t.dt.hour >= 0) & (t.dt.hour < 7)]
        london = g[(t.dt.hour >= 7) & (t.dt.hour < 13)]
        ...
        if asia_range_atr < float(params["asia_range_atr_min"]):
            continue
        london_close = float(london.iloc[-1]["close"])
...
def events_prev_day_range_extreme_fade(m15: pd.DataFrame, params: Dict[str, Any]) -> pd.DataFrame:
    x = m15.copy()
    x["day"] = pd.to_datetime(x["time"]).dt.date
    daily = (
        x.groupby("day")
    ...
    for i in range(1, len(days)):
        ...
        if float(prev["day_range_atr"]) > float(params["prev_day_range_atr_max"]):
            continue
        ...
        touched_high = float(pre_entry["high"].max()) >= float(prev["day_high"]) + ext
        touched_low = float(pre_entry["low"].min()) <= float(prev["day_low"]) - ext
        entry_close = float(entry["close"])
        if touched_high and not touched_low and entry_close > float(prev["day_high"]):
            direction = "short"
        elif touched_low and not touched_high and entry_close < float(prev["day_low"]):
            direction = "long"
        else:
            continue
        rows.append(
...
EVENT_GENERATORS = {
    "london_oneway_fade": events_london_oneway_fade,
    "asia_london_double_expansion_reversal": events_asia_london_double_expansion_reversal,
    "prev_day_range_extreme_fade": events_prev_day_range_extreme_fade,
}
...
def build_candidates() -> List[Candidate]:
    ...
    # v2 hotfix: tighter lite grid. This is discovery, not exhaustive optimization.
    # Keep enough diversity across three independent families while avoiding a
    # hundreds-of-candidates exact-like sweep.

    # Family 1: Intraday mean reversion after a one-way London move.
    for move_min in [0.9, 1.2]:
        for eff_min in [0.60, 0.72]:
            for cont_min in [0.0, 0.20]:
                    ...
                    for horizon in [90, 180]:
                        for tp_atr, sl_atr in [(0.40, 0.50), (0.60, 0.65)]:
                            params = {
                                ...
                                "eff_min": eff_min,
                                "ny_cont_atr_min": cont_min,
                                "entry_hour": entry_hour,
                                "horizon_min": horizon,
                                "tp_atr": tp_atr,
                                "sl_atr": sl_atr,
                            }
                            name = f"london_oneway_fade_mv{move_min}_eff{eff_min}_cont{cont_min}_h{horizon}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                            candidates.append(Candidate("london_oneway_fade", name, params))

    # Family 2: NY reversal after Asia/London double expansion.
    for asia_min in [0.45, 0.70]:
        for ext_min in [0.45, 0.75]:
            for entry_hour in [13, 15]:
                for horizon in [90, 180]:
                    for tp_atr, sl_atr in [(0.40, 0.50), (0.60, 0.65)]:
                        params = {
                            "asia_range_atr_min": asia_min,
                        ...
                        name = f"double_expansion_reversal_asia{asia_min}_ext{ext_min}_h{horizon}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                        candidates.append(Candidate("asia_london_double_expansion_reversal", name, params))

    # Family 3: Range-day extreme fade logic.
    for prev_range_max in [1.2, 1.7]:
        for extension_min in [0.15, 0.35]:
            for entry_hour in [13, 15]:
                for horizon in [90, 180]:
                    for tp_atr, sl_atr in [(0.40, 0.50), (0.60, 0.65)]:
                        params = {
                            "prev_day_range_atr_max": prev_range_max,
                        ...
                        name = f"prev_range_fade_rng{prev_range_max}_ext{extension_min}_h{horizon}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                        candidates.append(Candidate("prev_day_range_extreme_fade", name, params))
...
PARAM_KEYS_BY_FAMILY = {
    ...
    "asia_london_double_expansion_reversal": {
        "asia_range_atr_min",
    ...
    "prev_day_range_extreme_fade": {
        "prev_day_range_atr_max",
        "extension_atr_min",
        "entry_hour",
        "horizon_min",
        "tp_atr",
...
def render_markdown(report: Dict[str, Any], proxy_df: pd.DataFrame, exact_df: pd.DataFrame) -> str:
    ...
    lines = []
    lines.append(f"# Stage23A Lite Proxy-Then-Exact Discovery")
    lines.append("")
    ...
    lines.append("")
    lines.append("1. `london_oneway_fade` — intraday mean-reversion after a one-way London move.")
    lines.append("2. `asia_london_double_expansion_reversal` — NY reversal after Asia/London double expansion.")
    lines.append("3. `prev_day_range_extreme_fade` — range-day / previous-day extreme fade logic.")
    lines.append("")
    lines.append("## Counts")
    lines.append("")
    ...
    lines.append("")
    lines.append("Stage23A is discovery only. Continue operational forward-shadow collection separately:")
    lines.append("")
    lines.append("```bash")
    ...
    lines.append("")
    lines.append(f"- `{REPORT_DIR / 'stage23a_lite_proxy_exact_discovery.json'}`")
    lines.append(f"- `{REPORT_DIR / 'stage23a_lite_proxy_exact_discovery.md'}`")
    lines.append(f"- `{REPORT_DIR / 'stage23a_proxy_candidates.csv'}`")
    lines.append(f"- `{REPORT_DIR / 'stage23a_exact_candidates.csv'}`")
    lines.append(f"- `{REPORT_DIR / 'stage23a_exact_trades.csv'}`")
    lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    ...
    promotion_review = 0 if exact_df.empty else int((exact_df["decision"] == "STAGE23_PROMOTION_CANDIDATE_REVIEW_ONLY").sum())
    watchlist_only = 0 if exact_df.empty else int((exact_df["decision"] == "STAGE23_KEEP_WATCHLIST_ONLY").sum())
    if promotion_review > 0:
        decision = "STAGE23_HAS_PROMOTION_REVIEW_CANDIDATE_RESEARCH_ONLY"
    elif watchlist_only > 0:
        decision = "STAGE23_HAS_WATCHLIST_ONLY_CANDIDATES"
    else:
        decision = "STAGE23_NO_PROMOTION_KEEP_DISCOVERY_OPEN"
    ...
    proxy_path = REPORT_DIR / "stage23a_proxy_candidates.csv"
    exact_path = REPORT_DIR / "stage23a_exact_candidates.csv"
    trades_path = REPORT_DIR / "stage23a_exact_trades.csv"
    json_path = REPORT_DIR / "stage23a_lite_proxy_exact_discovery.json"
    md_path = REPORT_DIR / "stage23a_lite_proxy_exact_discovery.md"
