"""
Stage 24A — remaining-behavior discovery for XAUUSD.
...
- Build a lite M15 proxy grid over remaining behavior families not yet covered by Stage18A/Stage23D.
- Run exact M1 diagnostic replay for the best proxy candidates, while reserving runtime for exact.
- Write JSON/CSV/MD reports under data/reports/stage24a_remaining_behavior_discovery/.
...
STAGE_NAME = "stage24a_remaining_behavior_discovery"
...
MAX_EXACT_REPLAY_TOTAL = int(os.getenv("STAGE24A_MAX_EXACT", "12"))
MAX_EXACT_REPLAY_PER_FAMILY = int(os.getenv("STAGE24A_MAX_EXACT_PER_FAMILY", "4"))
MIN_PROXY_EVENTS = int(os.getenv("STAGE24A_MIN_PROXY_EVENTS", "25"))
MIN_EXACT_EVENTS = int(os.getenv("STAGE24A_MIN_EXACT_EVENTS", "35"))
MAX_RUNTIME_SECONDS = int(os.getenv("STAGE24A_MAX_RUNTIME_SECONDS", "180"))
EXACT_RESERVED_SECONDS = int(os.getenv("STAGE24A_EXACT_RESERVED_SECONDS", "45"))
MAX_CANDIDATES_TOTAL = int(os.getenv("STAGE24A_MAX_CANDIDATES_TOTAL", "84"))
PROXY_BOOTSTRAP_ITERS = int(os.getenv("STAGE24A_PROXY_BOOTSTRAP_ITERS", "20"))
EXACT_BOOTSTRAP_ITERS = int(os.getenv("STAGE24A_EXACT_BOOTSTRAP_ITERS", "80"))
# CSV-first avoids expensive SQLite table discovery on large local stores.
DATA_LOAD_MODE = os.getenv("STAGE24A_DATA_LOAD_MODE", "csv_first").strip().lower()
...
def decide_exact(summary: Dict[str, Any]) -> str:
    ...
    if events >= MIN_EXACT_EVENTS and pf_x1 >= 1.25 and pf_x4 >= 1.05 and test20 >= 1.05 and boot >= 1.0 and median > 0 and y_ok:
        return "STAGE24A_PROMOTION_CANDIDATE_REVIEW_ONLY"
    if events >= MIN_EXACT_EVENTS and pf_x1 >= 1.15 and pf_x4 >= 1.0 and test20 >= 0.95 and boot >= 0.85 and y_ok:
        return "STAGE24A_KEEP_WATCHLIST_ONLY"
    return "STAGE24A_REJECT"
...
def events_asia_compression_failed_breakout_fade(m15: pd.DataFrame, params: Dict[str, Any]) -> pd.DataFrame:
    """Fade a failed expansion after a compressed Asia range.

    Hypothesis: after a quiet Asia session, a small London/early-NY stop-run that
    returns back inside the Asia box is often a failed expansion rather than a
    clean breakout. This is distinct from Stage23A's one-way London fade and from
    Stage23B's one-way London continuation.
    """
    rows = []
    for _, g in _day_groups(m15):
        ...
        if asia_range_atr > float(params["asia_range_atr_max"]):
            continue
        t = pd.to_datetime(g["time"])
        pre_entry = g[(t.dt.hour >= 7) & (t <= pd.Timestamp(entry["time"]))]
        if pre_entry.empty:
            continue
        breakout = float(params["breakout_atr_min"]) * atr
        close = float(entry["close"])
        swept_high = float(pre_entry["high"].max()) >= asia_hi + breakout
        swept_low = float(pre_entry["low"].min()) <= asia_lo - breakout
        # Skip two-sided stop-runs; they are too ambiguous for a lite directional candidate.
        if swept_high and not swept_low and close < asia_hi:
            direction = "short"
        elif swept_low and not swept_high and close > asia_lo:
            direction = "long"
        else:
            continue
        rows.append(
...
def events_london_compression_expansion_continuation(m15: pd.DataFrame, params: Dict[str, Any]) -> pd.DataFrame:
    """Continuation after London compression resolves into a NY expansion."""
    rows = []
    for _, g in _day_groups(m15):
        ...
        london_range_atr = (london_hi - london_lo) / atr
        if london_range_atr > float(params["london_range_atr_max"]):
            continue
        close = float(entry["close"])
        breakout = float(params["breakout_close_atr_min"]) * atr
        if close > london_hi + breakout:
            direction = "long"
            extension_atr = (close - london_hi) / atr
        elif close < london_lo - breakout:
            direction = "short"
            extension_atr = (london_lo - close) / atr
        else:
            continue
        rows.append(
            {
...
def events_london_extreme_stoprun_reclaim(m15: pd.DataFrame, params: Dict[str, Any]) -> pd.DataFrame:
    """NY stop-run/reclaim around London extremes.

    Hypothesis: the NY session sometimes sweeps the London high/low and then
    reclaims back inside the London range; this tests a failed-expansion behavior
    around session extremes, not previous-day PDL/PDH sweep-reclaim.
    """
    rows = []
    for _, g in _day_groups(m15):
        london = _session_slice(g, 7, 13)
        ...
        london_lo = float(london["low"].min())
        london_range_atr = (london_hi - london_lo) / atr
        if london_range_atr < float(params["london_range_atr_min"]):
            continue
        t = pd.to_datetime(g["time"])
        pre_entry = g[(t.dt.hour >= 13) & (t <= pd.Timestamp(entry["time"]))]
        if pre_entry.empty:
            continue
        sweep = float(params["sweep_atr_min"]) * atr
        reclaim = float(params["reclaim_buffer_atr"]) * atr
        close = float(entry["close"])
        swept_high = float(pre_entry["high"].max()) >= london_hi + sweep
        swept_low = float(pre_entry["low"].min()) <= london_lo - sweep
        if swept_high and not swept_low and close < london_hi - reclaim:
            direction = "short"
        elif swept_low and not swept_high and close > london_lo + reclaim:
            direction = "long"
        else:
            continue
        rows.append(
...
EVENT_GENERATORS = {
    "asia_compression_failed_breakout_fade": events_asia_compression_failed_breakout_fade,
    "london_compression_expansion_continuation": events_london_compression_expansion_continuation,
    "london_extreme_stoprun_reclaim": events_london_extreme_stoprun_reclaim,
}
...
def build_candidates() -> List[Candidate]:
    ...
    # Family 1: quiet Asia -> failed expansion fade.
    for asia_max in [0.55, 0.85]:
        for breakout_min in [0.15, 0.30, 0.45]:
            for entry_hour in [13, 15]:
                for horizon in [90, 180]:
                    for tp_atr, sl_atr in [(0.40, 0.50), (0.60, 0.65)]:
                        params = {
                            "asia_range_atr_max": asia_max,
                            "breakout_atr_min": breakout_min,
                            "entry_hour": entry_hour,
                            "horizon_min": horizon,
                            "tp_atr": tp_atr,
                            "sl_atr": sl_atr,
                        }
                        name = f"asia_compression_failed_fade_asia{asia_max}_br{breakout_min}_h{horizon}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                        candidates.append(Candidate("asia_compression_failed_breakout_fade", name, params))

    # Family 2: London compression -> expansion continuation.
    for london_max in [0.60, 0.85, 1.10]:
        for breakout_min in [0.10, 0.25]:
            for entry_hour in [13, 15]:
                for horizon in [90, 180]:
                    for tp_atr, sl_atr in [(0.40, 0.50), (0.60, 0.65)]:
                        params = {
                            "london_range_atr_max": london_max,
                            "breakout_close_atr_min": breakout_min,
                            "entry_hour": entry_hour,
                            "horizon_min": horizon,
                            "tp_atr": tp_atr,
                            "sl_atr": sl_atr,
                        }
                        name = f"london_compression_expansion_cont_lmax{london_max}_br{breakout_min}_h{horizon}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                        candidates.append(Candidate("london_compression_expansion_continuation", name, params))

    # Family 3: NY stop-run/reclaim around London high/low.
    for range_min in [0.70, 1.00]:
        for sweep_min in [0.10, 0.25, 0.40]:
            for reclaim_buffer in [0.0, 0.10]:
                for entry_hour in [15, 16]:
                    for horizon in [90, 180]:
                        for tp_atr, sl_atr in [(0.40, 0.50), (0.60, 0.65)]:
                            params = {
                                "london_range_atr_min": range_min,
                                "sweep_atr_min": sweep_min,
                            ...
                            name = f"london_extreme_stoprun_reclaim_rng{range_min}_sw{sweep_min}_rec{reclaim_buffer}_h{horizon}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                            candidates.append(Candidate("london_extreme_stoprun_reclaim", name, params))

    # Deterministic interleaving: keep a controlled candidate cap while preventing
    # later families from being starved by earlier grid blocks.
    by_family: Dict[str, List[Candidate]] = {}
    for c in candidates:
        by_family.setdefault(c.family, []).append(c)
    ordered: List[Candidate] = []
    family_names = list(EVENT_GENERATORS.keys())
    offset = 0
    while len(ordered) < MAX_CANDIDATES_TOTAL:
        added = False
        for family in family_names:
            ...
            if offset < len(items):
                ...
                added = True
                if len(ordered) >= MAX_CANDIDATES_TOTAL:
                    break
        if not added:
            break
        offset += 1
...
PARAM_KEYS_BY_FAMILY = {
    "asia_compression_failed_breakout_fade": {
        "asia_range_atr_max",
        "breakout_atr_min",
        "entry_hour",
        "horizon_min",
    ...
    "london_compression_expansion_continuation": {
        "london_range_atr_max",
        "breakout_close_atr_min",
        "entry_hour",
        "horizon_min",
        "tp_atr",
        "sl_atr",
    },
    "london_extreme_stoprun_reclaim": {
        "london_range_atr_min",
        "sweep_atr_min",
        "reclaim_buffer_atr",
        "entry_hour",
        "horizon_min",
...
# ---------------------------------------------------------------------------
# Proxy grid and exact replay orchestration
# ---------------------------------------------------------------------------
...
def _candidate_from_row(row: pd.Series) -> Candidate:
    ...
    if isinstance(params_raw, str):
        ...
        params = {}
    return Candidate(family=str(row["family"]), name=str(row["name"]), params=params)


def _proxy_rank_score(summary: Dict[str, Any]) -> float:
    """Small deterministic ranking score for lite discovery.

    It deliberately rewards cost-aware PF and lower-tail robustness, while still
    allowing a few borderline candidates to reach exact diagnostic replay.
    """
    pf_x1 = float(summary.get("pf_x1", 0.0) or 0.0)
    pf_x4 = float(summary.get("pf_x4", 0.0) or 0.0)
    test20 = float(summary.get("test20_pf_x1", 0.0) or 0.0)
    boot = float(summary.get("boot_pf_p05_x1", 0.0) or 0.0)
    median = float(summary.get("median_x1", 0.0) or 0.0)
    events = max(0, int(summary.get("events", 0) or 0))
    event_bonus = min(1.5, math.log1p(events) / 4.0)
    median_bonus = max(-1.0, min(1.0, median / 2.0))
    return round(2.25 * pf_x1 + 2.75 * pf_x4 + 1.25 * test20 + 1.50 * boot + event_bonus + median_bonus, 4)


def run_proxy_grid(
    m15: pd.DataFrame,
    candidates: List[Candidate],
    proxy_deadline_ts: float,
) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame], bool]:
    """Run the controlled M15 proxy grid.

    Hotfix 1: this function was accidentally omitted from the first Stage24A
    patch. It is intentionally compact and runtime-capped. Event generation is
    cached by signal rule so variants that differ only by TP/SL/horizon do not
    regenerate identical signal rows.
    """
    ...
    for i, cand in enumerate(candidates):
        if time.monotonic() >= proxy_deadline_ts:
            proxy_timed_out = True
            break

        gen = EVENT_GENERATORS.get(cand.family)
        if gen is None:
            continue

        cache_key = _signal_cache_key(cand)
        if cache_key not in event_cache:
            try:
                events = gen(m15, cand.params)
            except Exception as exc:  # keep discovery robust; bad candidates should not kill the run
                events = pd.DataFrame()
                rows.append(
                    {
                        "family": cand.family,
                        ...
                        "median_x1": 0.0,
                        "total_x1": 0.0,
                        "proxy_rank_score": -999.0,
                        "error": f"event_generation_failed: {type(exc).__name__}: {exc}",
                    }
                )
        ...
        if events.empty or len(events) < MIN_PROXY_EVENTS:
            continue

        trades = replay_events(m15, events, cand, ROUNDTRIP_COST_USD)
        summary = summarize_trades(trades, stage="proxy_m15")
        if int(summary.get("events", 0) or 0) < MIN_PROXY_EVENTS:
            continue
        ...
        row = {
            ...
            "params_json": json.dumps(cand.params, sort_keys=True),
            **summary,
            "proxy_rank_score": _proxy_rank_score(summary),
        }
        rows.append(row)

    if not rows:
        return pd.DataFrame(), event_cache, proxy_timed_out

    df = pd.DataFrame(rows)
    # Keep highest score first; stable name tie-breaker makes output deterministic.
    df = df.sort_values(["proxy_rank_score", "pf_x4", "pf_x1", "events", "name"], ascending=[False, False, False, False, True]).reset_index(drop=True)
    return df, event_cache, proxy_timed_out


def select_exact_candidates(proxy_df: pd.DataFrame) -> List[Candidate]:
    """Select a small, family-balanced exact-replay set from proxy results."""
    if proxy_df.empty:
        return []
    ...
    for _, row in proxy_df.iterrows():
        if len(selected) >= MAX_EXACT_REPLAY_TOTAL:
            break
        cand = _candidate_from_row(row)
        if per_family.get(cand.family, 0) >= MAX_EXACT_REPLAY_PER_FAMILY:
            continue

        # Avoid exact-replaying exact duplicate signal rules with only TP/SL/horizon
        # differences until at least one representative has been selected. This keeps
        # Stage24A lite and prevents duplicate evidence inflation.
        sig_key = _signal_cache_key(cand)
        if sig_key in seen_signal_keys:
            continue

        selected.append(cand)
        seen_signal_keys.add(sig_key)
        per_family[cand.family] = per_family.get(cand.family, 0) + 1

    # If de-duplication was too strict, fill remaining slots from top rows.
    if len(selected) < min(MAX_EXACT_REPLAY_TOTAL, 3):
...
def run_exact_replay(
    m1: pd.DataFrame,
    exact_selection: List[Candidate],
    event_cache: Dict[str, pd.DataFrame],
    deadline_ts: float,
) -> Tuple[pd.DataFrame, pd.DataFrame, bool]:
    """Run exact M1 replay for selected candidates only."""
    rows: List[Dict[str, Any]] = []
    all_trades: List[Dict[str, Any]] = []
    exact_timed_out = False

    for cand in exact_selection:
        if time.monotonic() >= deadline_ts:
            exact_timed_out = True
            break

        events = event_cache.get(_signal_cache_key(cand), pd.DataFrame())
        if events.empty:
            continue

        trades = replay_events(m1, events, cand, ROUNDTRIP_COST_USD)
        summary = summarize_trades(trades, stage="exact_m1")
        ...
        for t in trades:
            rec = asdict(t)
            rec["stage24a_decision"] = decision
            all_trades.append(rec)

    exact_df = pd.DataFrame(rows)
    if not exact_df.empty:
        exact_df = exact_df.sort_values(["decision", "pf_x4", "pf_x1", "events", "name"], ascending=[True, False, False, False, True]).reset_index(drop=True)

    trades_df = pd.DataFrame(all_trades)
    return exact_df, trades_df, exact_timed_out
# ---------------------------------------------------------------------------
# Reporting
# ---------------------------------------------------------------------------
...
def render_markdown(report: Dict[str, Any], proxy_df: pd.DataFrame, exact_df: pd.DataFrame) -> str:
    ...
    lines = []
    lines.append(f"# Stage24A Remaining Behavior Discovery")
    lines.append("")
    ...
    lines.append("- Stage18A v2 and Stage23D are not modified. Stage19B/20A/21B/22A watchlist-only families are not touched or added to Stage18A.")
    lines.append("")
    lines.append("## Data")
    lines.append("")
    ...
    lines.append("")
    lines.append("1. `asia_compression_failed_breakout_fade` — quiet Asia range followed by failed expansion/reclaim.")
    lines.append("2. `london_compression_expansion_continuation` — London compression followed by NY expansion continuation.")
    lines.append("3. `london_extreme_stoprun_reclaim` — NY stop-run/reclaim around London extremes.")
    lines.append("")
    lines.append("## Counts")
    lines.append("")
    ...
    lines.append("")
    lines.append("Stage24A is discovery only. Continue Stage18A v2 and Stage23D forward-shadow collection separately:")
    lines.append("")
    lines.append("```bash")
    ...
    lines.append("")
    lines.append(f"- `{REPORT_DIR / 'stage24a_remaining_behavior_discovery.json'}`")
    lines.append(f"- `{REPORT_DIR / 'stage24a_remaining_behavior_discovery.md'}`")
    lines.append(f"- `{REPORT_DIR / 'stage24a_proxy_candidates.csv'}`")
    lines.append(f"- `{REPORT_DIR / 'stage24a_exact_candidates.csv'}`")
    lines.append(f"- `{REPORT_DIR / 'stage24a_exact_trades.csv'}`")
    lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    ...
    promotion_review = 0 if exact_df.empty else int((exact_df["decision"] == "STAGE24A_PROMOTION_CANDIDATE_REVIEW_ONLY").sum())
    watchlist_only = 0 if exact_df.empty else int((exact_df["decision"] == "STAGE24A_KEEP_WATCHLIST_ONLY").sum())
    if promotion_review > 0:
        decision = "STAGE24A_HAS_PROMOTION_REVIEW_CANDIDATE_RESEARCH_ONLY"
    elif watchlist_only > 0:
        decision = "STAGE24A_HAS_WATCHLIST_ONLY_CANDIDATES"
    else:
        decision = "STAGE24A_NO_PROMOTION_KEEP_DISCOVERY_OPEN"
    ...
    proxy_path = REPORT_DIR / "stage24a_proxy_candidates.csv"
    exact_path = REPORT_DIR / "stage24a_exact_candidates.csv"
    trades_path = REPORT_DIR / "stage24a_exact_trades.csv"
    json_path = REPORT_DIR / "stage24a_remaining_behavior_discovery.json"
    md_path = REPORT_DIR / "stage24a_remaining_behavior_discovery.md"
