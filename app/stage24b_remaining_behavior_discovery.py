...
Controlled proxy-then-exact discovery over remaining XAUUSD behavior families:
1) previous-day inside compression breakout continuation
2) NY open impulse continuation after London drive
3) Asia-London aligned continuation
...
REPORT_DIR = ROOT / "data" / "reports" / "stage24b_remaining_behavior_discovery"
...
ROUNDTRIP_COST_X1 = float(os.getenv("STAGE24B_ROUNDTRIP_COST_X1", "0.35"))
MAX_RUNTIME_SECONDS = int(os.getenv("STAGE24B_MAX_RUNTIME_SECONDS", "180"))
EXACT_RESERVED_SECONDS = int(os.getenv("STAGE24B_EXACT_RESERVED_SECONDS", "45"))
MAX_CANDIDATES_TOTAL = int(os.getenv("STAGE24B_MAX_CANDIDATES_TOTAL", "84"))
MAX_EXACT = int(os.getenv("STAGE24B_MAX_EXACT", "12"))
MAX_EXACT_PER_FAMILY = int(os.getenv("STAGE24B_MAX_EXACT_PER_FAMILY", "4"))
MIN_PROXY_EVENTS = int(os.getenv("STAGE24B_MIN_PROXY_EVENTS", "35"))
BOOT_SAMPLES = int(os.getenv("STAGE24B_BOOT_SAMPLES", "80"))
...
def load_data() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, str, str, str]:
    m1_override = _env_path("STAGE24B_M1_CSV")
    h1_override = _env_path("STAGE24B_H1_CSV")
    ...
    if not m1_path:
        raise FileNotFoundError("M1 CSV not found. Set STAGE24B_M1_CSV or place amarkets_xauusd_1m.csv in ~/Downloads.")
...
def candidate_grid() -> List[Candidate]:
    ...
    # Family 1: previous-day compression/inside range breakout continuation.
    for inside_ratio in [0.55, 0.70, 0.85]:
        for confirm_atr in [0.05, 0.10]:
            for entry_hour in [13, 15]:
                for horizon_min in [90, 180]:
                    for tp_atr, sl_atr in [(0.6, 0.65), (0.8, 0.8)]:
                        name = f"inside_break_cont_in{inside_ratio}_cf{confirm_atr}_h{horizon_min}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                        cands.append(Candidate("prev_day_inside_breakout_continuation", name, dict(
    ...
    # Family 2: London drive + NY confirmation continuation.
    for london_move_atr in [0.45, 0.70, 1.00]:
        for ny_confirm_atr in [0.10, 0.20]:
            for entry_hour in [13, 14]:
                for horizon_min in [90, 180]:
                    for tp_atr, sl_atr in [(0.6, 0.65), (0.8, 0.8)]:
                        name = f"ny_impulse_cont_lm{london_move_atr}_cf{ny_confirm_atr}_h{horizon_min}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                        cands.append(Candidate("ny_open_impulse_continuation", name, dict(
                            london_move_atr=london_move_atr,
                            ny_confirm_atr=ny_confirm_atr,
                            entry_hour=entry_hour,
                            horizon_min=horizon_min,
                            tp_atr=tp_atr,
                            sl_atr=sl_atr,
    ...
    # Family 3: Asia and London aligned directional pressure continuation.
    for asia_move_atr in [0.20, 0.35]:
        for london_move_atr in [0.45, 0.70]:
            for entry_hour in [13, 15]:
                for horizon_min in [90, 180]:
                    for tp_atr, sl_atr in [(0.6, 0.65)]:
                        name = f"asia_london_align_am{asia_move_atr}_lm{london_move_atr}_h{horizon_min}_e{entry_hour}_tp{tp_atr}_sl{sl_atr}"
                        cands.append(Candidate("asia_london_alignment_continuation", name, dict(
                            asia_move_atr=asia_move_atr,
                            london_move_atr=london_move_atr,
                            entry_hour=entry_hour,
                            horizon_min=horizon_min,
                            tp_atr=tp_atr,
...
def generate_events_for_candidate(m15: pd.DataFrame, daily: pd.DataFrame, cand: Candidate) -> List[Event]:
    ...
    for date, day in x.groupby("date", sort=True):
        ...
        if cand.family == "prev_day_inside_breakout_continuation":
            prev_range = float(d.get("prev_range", np.nan))
            prev2_range = float(d.get("prev2_range", np.nan))
            prev_high = float(d.get("prev_high", np.nan))
            prev_low = float(d.get("prev_low", np.nan))
            if not all(np.isfinite(v) for v in [prev_range, prev2_range, prev_high, prev_low]):
                continue
            if prev2_range <= 0 or prev_range > float(p["inside_ratio"]) * prev2_range:
                continue
            close = float(entry_bar["close"])
            if close >= prev_high + float(p["confirm_atr"]) * atr:
                direction = 1
            elif close <= prev_low - float(p["confirm_atr"]) * atr:
                direction = -1
            meta = {"prev_range": prev_range, "prev2_range": prev2_range}

        elif cand.family == "ny_open_impulse_continuation":
            ...
            if abs(london_move) < float(p["london_move_atr"]) * atr:
                continue
            dir0 = 1 if london_move > 0 else -1
            confirm_move = float(entry_bar["close"] - entry_bar["open"])
            if dir0 * confirm_move < float(p["ny_confirm_atr"]) * atr:
                continue
            direction = dir0
            meta = {"london_move": london_move, "confirm_move": confirm_move}

        elif cand.family == "asia_london_alignment_continuation":
            asia = day[(day["hour"] >= 0) & (day["hour"] < 7)]
            london = day[(day["hour"] >= 7) & (day["hour"] < entry_hour)]
            if asia.empty or london.empty:
                continue
            asia_move = float(asia.iloc[-1]["close"] - asia.iloc[0]["open"])
            london_move = float(london.iloc[-1]["close"] - london.iloc[0]["open"])
            if abs(asia_move) < float(p["asia_move_atr"]) * atr:
                continue
            if abs(london_move) < float(p["london_move_atr"]) * atr:
                continue
            if np.sign(asia_move) != np.sign(london_move):
                continue
            direction = 1 if london_move > 0 else -1
            meta = {"asia_move": asia_move, "london_move": london_move}
...
def decision_for_exact(row: Dict[str, Any]) -> str:
    if row["events"] >= 50 and row["pf_x4"] >= 1.25 and row["boot_pf_p05_x1"] >= 1.0 and row["test20_pf_x1"] >= 1.0 and row["total_x1"] > 0:
        return "STAGE24B_PROMOTION_CANDIDATE_REVIEW_ONLY"
    if row["events"] >= 50 and row["pf_x1"] >= 1.10 and row["total_x1"] > 0:
        return "STAGE24B_KEEP_WATCHLIST_ONLY"
    return "STAGE24B_REJECT"
...
def render_markdown(report: Dict[str, Any], proxy_df: pd.DataFrame, exact_df: pd.DataFrame) -> str:
    lines: List[str] = []
    lines.append("# Stage24B Remaining Behavior Discovery")
    lines.append("")
    ...
    lines.append("")
    lines.append("1. `prev_day_inside_breakout_continuation` — previous-day compression/inside range followed by breakout continuation.")
    lines.append("2. `ny_open_impulse_continuation` — London directional drive plus NY confirmation.")
    lines.append("3. `asia_london_alignment_continuation` — Asia and London aligned directional pressure continued into NY.")
    lines.append("")
    lines.append("## Counts")
    lines.append("")
    ...
    lines.append("- If no promotion appears, close Stage24B and continue with a new behavior cluster rather than widening this grid indefinitely.")
    lines.append("")
    ...
    lines.append("")
    lines.append("- `data/reports/stage24b_remaining_behavior_discovery/stage24b_remaining_behavior_discovery.json`")
    lines.append("- `data/reports/stage24b_remaining_behavior_discovery/stage24b_remaining_behavior_discovery.md`")
    lines.append("- `data/reports/stage24b_remaining_behavior_discovery/stage24b_proxy_candidates.csv`")
    lines.append("- `data/reports/stage24b_remaining_behavior_discovery/stage24b_exact_candidates.csv`")
    lines.append("- `data/reports/stage24b_remaining_behavior_discovery/stage24b_exact_trades.csv`")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    promotion_count = 0 if exact_df.empty else int((exact_df["decision"] == "STAGE24B_PROMOTION_CANDIDATE_REVIEW_ONLY").sum())
    watchlist_count = 0 if exact_df.empty else int((exact_df["decision"] == "STAGE24B_KEEP_WATCHLIST_ONLY").sum())
    if promotion_count > 0:
        decision = "STAGE24B_HAS_PROMOTION_REVIEW_CANDIDATE_RESEARCH_ONLY"
    elif watchlist_count > 0:
        decision = "STAGE24B_WATCHLIST_ONLY_KEEP_DISCOVERY_OPEN"
    else:
        decision = "STAGE24B_NO_PROMOTION_KEEP_DISCOVERY_OPEN"
    ...
    proxy_df.to_csv(REPORT_DIR / "stage24b_proxy_candidates.csv", index=False)
    exact_df.to_csv(REPORT_DIR / "stage24b_exact_candidates.csv", index=False)
    trades_df.to_csv(REPORT_DIR / "stage24b_exact_trades.csv", index=False)
    (REPORT_DIR / "stage24b_remaining_behavior_discovery.json").write_text(json.dumps(json_safe(report), indent=2), encoding="utf-8")
    (REPORT_DIR / "stage24b_remaining_behavior_discovery.md").write_text(render_markdown(report, proxy_df, exact_df), encoding="utf-8")
    print(f"Decision: {decision}")
    print(f"Report: {REPORT_DIR / 'stage24b_remaining_behavior_discovery.md'}")
