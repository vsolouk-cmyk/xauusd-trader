#!/usr/bin/env python3
"""
Stage81B Corrected Hard Audit for Stage80 structured thesis discovery shortlist.
...
def evaluate_rule(df: pd.DataFrame, rule: Rule, date_col: str, price_col: str) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    # Stage81B correction:
    # Stage81 counted every row with a NaN in a derived trigger column as a hard
    # missing-feature failure. That incorrectly penalized the natural warm-up
    # window for derived features such as 20d/60d returns and SMA spreads.
    # Count only unexpected missing rows after the first row where all required
    # trigger/date/price columns are simultaneously available. If no complete row
    # exists, the whole dataset is unusable for this rule.
    if required_cols:
        complete_mask = df[required_cols].notna().all(axis=1)
        if complete_mask.any():
            first_complete_idx = int(complete_mask[complete_mask].index[0])
            unexpected_missing_mask = df.loc[first_complete_idx:, required_cols].isna().any(axis=1)
            missing_required_feature_rows = int(unexpected_missing_mask.sum())
            warmup_missing_rows_ignored = int(df.loc[:first_complete_idx - 1, required_cols].isna().any(axis=1).sum()) if first_complete_idx > 0 else 0
        else:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if selected_rows:
        decision = "STAGE81B_HARD_AUDIT_SHORTLIST_READY_FOR_PORTFOLIO_REVIEW_NO_ORDER"
        classification = "S81B_HARD_AUDIT_SHORTLIST_READY"
        disposition = "HARD_AUDIT_SHORTLIST_READY_FOR_PORTFOLIO_REVIEW"
    else:
        decision = "STAGE81B_NO_DISCOVERY_CANDIDATE_SURVIVES_HARD_AUDIT_NO_ORDER"
        classification = "S81B_NO_HARD_AUDIT_SURVIVOR"
        disposition = "NO_DISCOVERY_CANDIDATE_SURVIVES_HARD_AUDIT"

    metrics_csv = out_dir / "stage81b_corrected_hard_audit_metrics.csv"
    selected_csv = out_dir / "stage81b_selected_for_stage82.csv"
    failures_csv = out_dir / "stage81b_corrected_hard_audit_failures.csv"
    entries_csv = out_dir / "stage81b_corrected_hard_audit_entry_returns.csv"
    summary_json = out_dir / "stage81b_corrected_hard_audit_stage80_shortlist_summary.json"
    report_md = out_dir / "stage81b_corrected_hard_audit_stage80_shortlist_report.md"
    ...
    summary = {
        "stage": "Stage81B_CORRECTED_HARD_AUDIT_STAGE80_SHORTLIST",
        "root": str(root),
        "config": str(rel(root, args.config)),
        "generated_utc": utc_now(),
        "status": "STAGE81B_COMPLETE_NO_PROMOTION",
        "decision": decision,
        ...
        "hard_blocks": [
            ...
            "NO_PAPER_LIVE",
            "NO_LIVE",
            "NO_ORDER_AUTHORIZATION_FROM_STAGE81B",
            "NO_THRESHOLD_TUNING_FROM_STAGE81B_HARD_AUDIT",
            "NO_DIRECT_MT5_OR_EA_CHANGE_FROM_STAGE81B",
        ],
        "outputs": {
    ...
    lines = [
        ...
        "",
        "## Decision",
        f"- status: `STAGE81B_COMPLETE_NO_PROMOTION`",
        f"- decision: `{decision}`",
