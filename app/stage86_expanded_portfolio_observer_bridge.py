...
HARD_BLOCKS = [
    ...
    "NO_PAPER_LIVE",
    "NO_LIVE",
    "NO_ORDER_AUTHORIZATION_FROM_STAGE86",
    "NO_THRESHOLD_TUNING_FROM_EXPANDED_PORTFOLIO_BRIDGE",
]
...
def main() -> None:
    ...
    pairs: List[Tuple[str, Any]] = [
        ("schema_version", "stage86_expanded_portfolio_observer_v1"),
        ("stage", "Stage86_EXPANDED_PORTFOLIO_OBSERVER_BRIDGE"),
    ...
    decision = "STAGE86_EXPANDED_PORTFOLIO_OBSERVER_READY_ACTIVE_REVIEW_ONLY_NO_ORDER" if active_rules else "STAGE86_EXPANDED_PORTFOLIO_OBSERVER_READY_WAIT_SIGNAL_NO_ORDER"
    if issues:
        decision = "STAGE86_EXPANDED_PORTFOLIO_OBSERVER_BLOCKED_NO_ORDER"
    ...
    summary: Dict[str, Any] = {
        ...
        "generated_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "status": "STAGE86_COMPLETE_NO_PROMOTION",
        "decision": decision,
        "classification": "S86_EXPANDED_PORTFOLIO_OBSERVER_READY" if not issues else "S86_EXPANDED_PORTFOLIO_OBSERVER_BLOCKED",
        "disposition": "EXPANDED_PORTFOLIO_OBSERVER_READY_NO_ORDER" if not issues else "EXPANDED_PORTFOLIO_OBSERVER_BLOCKED_NO_ORDER",
        "bridge_role": "MT5 expanded portfolio observer-only bridge. The EA may read/display signals but cannot trade.",
        "macro_dataset": {
            "path": str(macro_path),
            "rows": len(rows),
            "date_col": date_col,
        ...
        "expanded_portfolio_rule_ids": [r["rule_id"] for r in RULES],
        ...
        "outputs": {
            "summary_json": str(out_dir / "stage86_expanded_portfolio_observer_bridge_summary.json"),
            "report_md": str(out_dir / "stage86_expanded_portfolio_observer_bridge_report.md"),
            "bridge_csv": str(bridge_csv),
        },
    }

    summary_path = out_dir / "stage86_expanded_portfolio_observer_bridge_summary.json"
    report_path = out_dir / "stage86_expanded_portfolio_observer_bridge_report.md"
    ...
    lines = [
        ...
        "",
        "## Latest expanded portfolio signal",
