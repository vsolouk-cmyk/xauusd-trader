#!/usr/bin/env python3
"""Stage88 daily unified observer combo runner.

Runs the local macro refresh stages and the Stage87 unified observer bridge,
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    lines: List[str] = []
    lines.append("# Stage88 Daily Unified Observer Combo")
    lines.append("")
    ...
    for k in ["feature_date", "latest_feature_date_utc", "mode", "any_signal_active", "selected_rule_id", "K06_primary_active", "K06_signal_active", "S83_14_signal_active", "S83_13_signal_active"]:
        if k in snap:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ok = not issues
    summary = {
        "stage": "Stage88_DAILY_UNIFIED_OBSERVER_COMBO",
        "root": str(root),
        "config": str(config_path),
        "generated_utc": utc_now(),
        "status": "STAGE88_COMPLETE_NO_PROMOTION" if ok else "STAGE88_COMPLETE_WITH_ISSUES_NO_PROMOTION",
        "decision": "STAGE88_DAILY_UNIFIED_COMBO_COMPLETE_OBSERVER_READY_NO_ORDER" if ok else "STAGE88_DAILY_UNIFIED_COMBO_ISSUES_NO_ORDER",
        "classification": "S88_UNIFIED_OBSERVER_READY" if ok else "S88_UNIFIED_OBSERVER_ISSUES",
        "disposition": "DAILY_UNIFIED_COMBO_OBSERVER_READY_NO_ORDER" if ok else "DAILY_UNIFIED_COMBO_OBSERVER_ISSUES_NO_ORDER",
        "combo_role": "Local daily operator combo for macro refresh and unified observer-only MT5 bridge generation.",
        ...
        "operator_instructions": [
            "Stage88 cannot authorize orders.",
            "Use --skip-refresh when source files have not changed and only the unified observer CSV must be rebuilt.",
        ...
        "outputs": {
            "summary_json": str(out_dir / "stage88_daily_unified_observer_combo_summary.json"),
            "report_md": str(out_dir / "stage88_daily_unified_observer_combo_report.md"),
            "unified_csv": str(csv_path),
        },
    }
    write_json(out_dir / "stage88_daily_unified_observer_combo_summary.json", summary)
    write_report(out_dir / "stage88_daily_unified_observer_combo_report.md", summary)
    print(json.dumps({
        "stage": summary["stage"],
