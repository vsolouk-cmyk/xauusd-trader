...
REPORT_REL = Path("reports/xauusd_cross_asset_fixed_scan")
PANEL_REPORT_REL = Path("reports/xauusd_cross_asset_intraday_panel")
CONFIG_REL = Path("configs/xauusd_cross_asset_fixed_scan.json")
...
def verify_panel(root: Path) -> dict[str, Any]:
    ...
    if summary.get("decision") != "PASS_CROSS_ASSET_INTRADAY_CAUSAL_PANEL_READY_FOR_FIXED_SCAN":
        raise ScanError(f"panel decision not fixed-scan ready: {summary.get('decision')}")
...
def required_columns(config: dict[str, Any]) -> tuple[list[str], list[str]]:
    base_features = {
        ...
        "official_event_blackout_active",
    }
    ...
    for h in sorted({int(x["horizon_hours"]) for x in config["candidates"]}):
        target_cols.update({
            f"exit_time_{h}h_utc", f"forward_return_{h}h_bps",
        })
...
def load_panel_data(root: Path, config: dict[str, Any]) -> pd.DataFrame:
    paths = panel_paths(root)
...
def common_eligibility(frame: pd.DataFrame, horizon: int, config: dict[str, Any]) -> pd.Series:
    ...
    mask = (
        ...
        & (pd.to_numeric(frame["official_event_blackout_active"], errors="coerce").fillna(0) == 0)
...
def matched_bucket_excess(
    ...
    target_col = f"forward_return_{horizon}h_bps"
    base = frame.loc[universe, ["decision_time_utc", target_col]].copy()
    ...
    bucket = base.groupby(["year", "hour_utc"], dropna=False)["baseline_net"].mean()
    keys = list(zip(trades["year"].astype(int), trades["hour_utc"].astype(int)))
...
def static_execution_violations(root: Path) -> list[str]:
    paths = [root / "app/xauusd_cross_asset_fixed_scan.py", root / CONFIG_REL]
...
def preflight(root: Path, config_path: Path) -> dict[str, Any]:
    ...
    return {
        "program": PROGRAM,
        "generated_utc": utc_now(),
        "decision": "PASS_CROSS_ASSET_FIXED_SCAN_PREFLIGHT_REFERENCE_ONLY",
        "pass": True,
        "paper_order_allowed": False,
        "demo_order_allowed": False,
        "live_order_allowed": False,
        ...
        "required_next_action": "RUN_FIXED_REFERENCE_SCAN_ONCE",
    }
...
def run(root: Path, config_path: Path, out: Path | None = None) -> dict[str, Any]:
    ...
    write_json(report / "cross_asset_fixed_scan_run_lock.json", {
        "program": PROGRAM,
        "created_utc": utc_now(),
        "status": "RUN_STARTED_FAIL_CLOSED",
    })
    set_run_context("LOAD_PANEL_DATA")
    frame = load_panel_data(root, config)
    ...
    write_json(report / "cross_asset_candidate_registry.json", registry)
    input_contract = {
        "program": PROGRAM,
        "generated_utc": utc_now(),
    ...
    write_json(report / "cross_asset_fixed_scan_input_contract.json", input_contract)
    ...
    metrics_df.to_csv(report / "cross_asset_reference_metrics.csv", index=False)
    folds_df.to_csv(report / "cross_asset_reference_fold_metrics.csv", index=False)
    annual_df.to_csv(report / "cross_asset_reference_annual_metrics.csv", index=False)
    set_run_context("WRITE_REFERENCE_OUTPUTS")
    write_csv_gz(report / "cross_asset_reference_trades.csv.gz", trades_df)
    ...
    if survivor_candidates:
        ...
        for candidate_id in ranked_ids:
            ...
            if len(shortlist) >= int(config["maximum_reference_survivors_for_diagnostic"]):
                break
        write_json(report / "cross_asset_reference_survivor_contract.json", {
            "program": PROGRAM,
            "generated_utc": utc_now(),
            "selection_used_2025_plus": False,
            "survivors": shortlist,
            "reference_metrics_sha256": sha256_file(report / "cross_asset_reference_metrics.csv"),
            "status": "FROZEN_BEFORE_DIAGNOSTIC",
        })
        write_json(report / "cross_asset_diagnostic_access_lock.json", {
            "program": PROGRAM,
            "generated_utc": utc_now(),
            ...
            "survivor_contract_sha256": sha256_file(report / "cross_asset_reference_survivor_contract.json"),
        })
        ...
        diagnostic_metrics.to_csv(report / "cross_asset_diagnostic_metrics.csv", index=False)
        write_csv_gz(report / "cross_asset_diagnostic_trades.csv.gz", diagnostic_trades)

    if not survivor_candidates:
        decision = "NO_REFERENCE_SURVIVOR_REJECT_FIXED_CROSS_ASSET_SET"
        required_next = "REJECT_FIXED_SET_AND_REASSESS_INTRADAY_INFORMATION_CONTENT"
    elif (
        ...
        decision = "REFERENCE_SURVIVOR_DIAGNOSTIC_BREAKDOWN_REJECT_FORMULATION"
        required_next = "DO_NOT_RETUNE_ON_2025_PLUS"
    else:
        decision = "REFERENCE_SURVIVOR_READY_FOR_INDEPENDENT_FEED_REPLICATION"
        required_next = "REPLICATE_FROZEN_SURVIVOR_ON_INDEPENDENT_FEED_AND_STRICT_COST_REPLAY"

    summary = {
        "program": PROGRAM,
        "generated_utc": utc_now(),
        "decision": decision,
        "pass": decision == "REFERENCE_SURVIVOR_READY_FOR_INDEPENDENT_FEED_REPLICATION",
        "paper_order_allowed": False,
        "demo_order_allowed": False,
        "live_order_allowed": False,
    ...
    write_json(report / "cross_asset_fixed_scan_summary.json", summary)
    decision_lines = [
        "# XAUUSD Fixed Cross-Asset Intraday Thesis Scan",
        "",
        f"Decision: `{decision}`",
        "",
        f"Reference candidates: {len(config['candidates'])}",
        f"Reference survivors: {int(metrics_df['reference_pass'].sum())}",
    ...
    (report / "cross_asset_fixed_scan_decision.md").write_text("\n".join(decision_lines) + "\n", encoding="utf-8")
    lock = read_json(report / "cross_asset_fixed_scan_run_lock.json")
    lock["status"] = "RUN_COMPLETED"
    lock["completed_utc"] = utc_now()
    lock["decision"] = decision
    write_json(report / "cross_asset_fixed_scan_run_lock.json", lock)
...
def collect(root: Path, out: Path | None = None) -> Path:
    ...
    summary_path = report / "cross_asset_fixed_scan_summary.json"
    if not summary_path.is_file():
        raise ScanError(f"fixed scan summary missing: {summary_path}")
    output = out or (Path.home() / "Downloads" / "XAUUSD_CROSS_ASSET_FIXED_SCAN_RESULTS.zip")
    names = [
        "cross_asset_fixed_scan_summary.json",
        "cross_asset_fixed_scan_decision.md",
        "cross_asset_fixed_scan_input_contract.json",
        "cross_asset_fixed_scan_run_lock.json",
        "cross_asset_candidate_registry.json",
        "cross_asset_reference_metrics.csv",
        "cross_asset_reference_fold_metrics.csv",
        "cross_asset_reference_annual_metrics.csv",
        "cross_asset_reference_trades.csv.gz",
        "cross_asset_reference_survivor_contract.json",
        "cross_asset_diagnostic_access_lock.json",
        "cross_asset_diagnostic_metrics.csv",
        "cross_asset_diagnostic_trades.csv.gz",
    ]
    files = [report / name for name in names if (report / name).is_file()]
    manifest = {
        "program": PROGRAM,
        "generated_utc": utc_now(),
        "artifact_scope": "FIXED_SCAN_EVIDENCE_NO_LARGE_PANEL_INPUTS",
        "files": [
...
def failure_payload(exc: Exception) -> dict[str, Any]:
    return {
        "program": PROGRAM,
        "generated_utc": utc_now(),
        "decision": "CROSS_ASSET_FIXED_SCAN_FAIL_CLOSED",
        "pass": False,
        "paper_order_allowed": False,
        "demo_order_allowed": False,
        "live_order_allowed": False,
    ...
    try:
        if args.command == "preflight":
            ...
            print(json.dumps({
                "program": PROGRAM,
                "generated_utc": utc_now(),
                "decision": "PASS_CROSS_ASSET_FIXED_SCAN_RESULTS_PACK_CREATED",
                "pass": True,
                "paper_order_allowed": False,
                "demo_order_allowed": False,
                "live_order_allowed": False,
        ...
        report = root / REPORT_REL
        report.mkdir(parents=True, exist_ok=True)
        write_json(report / "cross_asset_fixed_scan_failure.json", payload)
        print(json.dumps(payload, indent=2))
        return 2
