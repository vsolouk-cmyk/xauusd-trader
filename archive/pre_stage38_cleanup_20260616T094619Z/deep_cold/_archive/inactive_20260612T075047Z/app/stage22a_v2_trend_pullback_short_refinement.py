...
Why v2:
- Full Stage22A exact grid can run too long.
- v2 uses a two-phase design:
    Phase 1: fast M15 proxy replay for all parameter variants.
    Phase 2: exact M1 replay only for the top-ranked proxy variants.
- Bootstrap is computed only on exact finalists, not on every grid row.
...
TOOL_VERSION = "v2_hybrid_proxy_then_exact"
DEFAULT_DB = Path("data/local/xauusd_local_store.sqlite")
DEFAULT_OUT_DIR = Path("data/reports/stage22a_v2_trend_pullback_short_refinement")
...
def build_variants(fast: bool = False) -> List[Variant]:
    if fast:
        trend_dists = [7.5, 10.0]
        pullbacks = [0.5, 1.0]
        slope_mins = [2.0]
        sma50_dists = [5.0]
        horizons = [8, 12]
    else:
        trend_dists = [5.0, 7.5, 10.0, 12.5]
        pullbacks = [0.0, 0.5, 1.0, 2.0]
        slope_mins = [0.0, 2.0, 5.0]
        sma50_dists = [0.0, 5.0, 10.0]
        horizons = [4, 8, 12, 16]
            ...
            for sm in slope_mins:
                for sd in sma50_dists:
                    for h in horizons:
                        ...
                        variants.append(Variant(name, td, pb, sm, sd, h))
    return variants
...
def replay_m15_proxy(v: Variant, ctx: pd.DataFrame, cost_usd: float) -> pd.DataFrame:
    ...
    for i in idxs:
        ...
        if entry_i >= len(ctx) or exit_i >= len(ctx):
            continue
        entry_price = float(ctx.iloc[entry_i]["open"])
        exit_price = float(ctx.iloc[exit_i]["close"])
        gross = entry_price - exit_price
        rows.append({
            "variant": v.name,
            "entry_dt": pd.to_datetime(ctx.iloc[entry_i]["dt"], utc=True),
            "net_x1": gross - cost_usd,
            "net_x2": gross - 2 * cost_usd,
            "net_x4": gross - 4 * cost_usd,
        })
...
def replay_exact_m1(v: Variant, ctx: pd.DataFrame, m15: pd.DataFrame, m1: pd.DataFrame, cost_usd: float) -> pd.DataFrame:
    mask = signal_mask(ctx, v)
    idxs = apply_spacing(ctx.index[mask].tolist(), v.horizon_bars, v.cooldown_bars)
    rows = []
    for i in idxs:
...
def bootstrap(df: pd.DataFrame, col: str = "net_x1", n: int = 100, seed: int = 220) -> Dict:
    if df.empty:
...
def proxy_score(row: Dict) -> float:
    score = 0.0
    score += min(8, float(row.get("pf_x1", 0)) * 2.5)
    score += min(5, max(0.0, float(row.get("median_x1", 0))) * 1.2)
    score += min(4, max(0.0, float(row.get("total_x1", 0))) / 100.0)
    score += min(4, max(0.0, float(row.get("total_2026", 0))) / 80.0)
    score -= min(6, abs(min(0.0, float(row.get("dd_x1", 0)))) / 200.0)
    return round(score, 6)


def summarize_proxy(v: Variant, trades: pd.DataFrame) -> Dict:
    ...
    if (
        ...
        decision = "PROXY_EXACT_CANDIDATE"
...
def exact_decide(m1: Dict, m2: Dict, m4: Dict, s80: Dict, s70: Dict, y2026: Dict, boot: Dict) -> Tuple[str, str, float]:
    score = 0.0
    ...
    if watch:
        return "KEEP_REFINED_WATCHLIST", "Positive exact replay but still not robust enough for forward-shadow design.", round(score, 6)
    return "REJECT_REFINED_WEAK", "Exact replay does not preserve enough robust edge.", round(score, 6)


def summarize_exact(v: Variant, trades: pd.DataFrame, bootstrap_n: int) -> Dict:
    m1 = metrics(trades, "net_x1")
    m2 = metrics(trades, "net_x2")
    m4 = metrics(trades, "net_x4")
    ...
    decision, reason, score = exact_decide(m1, m2, m4, s80, s70, y2026, boot)
    return {
        "variant": v.name,
        "family": "trend_pullback_short_refined",
        "side": "SHORT",
...
def run(db: Path, out_dir: Path, cost_usd: float, bootstrap_n: int, exact_top_n: int, fast: bool, top_n_trades: int) -> int:
    generated = now_iso()
    ...
    proxy_rows = []
    for v in variants:
        proxy_rows.append(summarize_proxy(v, replay_m15_proxy(v, ctx, cost_usd=cost_usd)))

    proxy = pd.DataFrame(proxy_rows)
    proxy = proxy.sort_values(["decision", "score", "pf_x1", "events"], ascending=[True, False, False, False])
    candidates = proxy[proxy["decision"].eq("PROXY_EXACT_CANDIDATE")].copy()
    if candidates.empty:
        candidates = proxy.head(max(1, exact_top_n)).copy()
    else:
        candidates = candidates.head(max(1, exact_top_n)).copy()

    exact_rows = []
    trade_parts = []
    for name in candidates["variant"].tolist():
        v = variant_by_name[name]
        trades = replay_exact_m1(v, ctx, m15, m1, cost_usd=cost_usd)
        exact_rows.append(summarize_exact(v, trades, bootstrap_n=bootstrap_n))
        if not trades.empty:
    ...
    exact["_order"] = exact["decision"].map(order).fillna(9)
    exact = exact.sort_values(["_order", "score", "pf_x1", "events"], ascending=[True, False, False, False]).drop(columns=["_order"])

    promoted = exact[exact["decision"].eq("PROMOTE_FORWARD_SHADOW_DESIGN_CANDIDATE")].copy()
    watch = exact[exact["decision"].eq("KEEP_REFINED_WATCHLIST")].copy()
    ...
    if top_n_trades > 0 and not all_trades.empty:
        keep = exact.head(top_n_trades)["variant"].tolist()
    ...
    proxy_csv = out_dir / "stage22a_v2_proxy_summary.csv"
    exact_csv = out_dir / "stage22a_v2_exact_summary.csv"
    promoted_csv = out_dir / "stage22a_v2_promoted.csv"
    trades_csv = out_dir / "stage22a_v2_exact_trades.csv"
    json_path = out_dir / "stage22a_v2_trend_pullback_short_refinement.json"
    md_path = out_dir / "stage22a_v2_trend_pullback_short_refinement.md"

    proxy.to_csv(proxy_csv, index=False)
    ...
    payload = {
        ...
        "source": {
            ...
            "variants_total": int(len(proxy)),
            "variants_exact_replayed": int(len(exact)),
        },
        "final_decision": final_decision,
        "counts": {
            "proxy_exact_candidates": int(len(proxy[proxy["decision"].eq("PROXY_EXACT_CANDIDATE")])),
            "exact_replayed": int(len(exact)),
    ...
    lines = [
        ...
        f"- exact_top_n: `{exact_top_n}`",
        f"- fast: `{fast}`",
        f"- variants_total: `{len(proxy)}`",
        f"- variants_exact_replayed: `{len(exact)}`",
        "",
        "## Final decision",
        f"- final_decision: `{final_decision}`",
        "",
        "## Counts",
        f"- proxy_exact_candidates: `{len(proxy[proxy['decision'].eq('PROXY_EXACT_CANDIDATE')])}`",
        f"- exact_replayed: `{len(exact)}`",
    ...
    for i, r in enumerate(exact.head(30).to_dict(orient="records"), 1):
        lines.append(
    ...
    print("Stage 22A v2 hybrid refinement: DONE")
    print(f"final_decision={final_decision}")
    print(f"variants_total={len(proxy)} exact_replayed={len(exact)} promoted={len(promoted)} watchlist={len(watch)}")
...
def main() -> int:
    ...
    p.add_argument("--exact-top-n", type=int, default=30)
    ...
    return run(
        ...
        exact_top_n=int(args.exact_top_n),
        fast=bool(args.fast),
