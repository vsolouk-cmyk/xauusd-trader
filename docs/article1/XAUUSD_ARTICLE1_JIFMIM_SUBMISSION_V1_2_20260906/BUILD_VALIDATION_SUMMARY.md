...
- The prose was revised throughout; the main article has no manual bold/italic emphasis commands and no body sentence beginning with `No` or `Not`.
- Section 3 is titled `Research design, data, and implementation`; protocol fixation is described within the chronology rather than promoted as a method in the heading.
- The software subsection now documents the five strategy engines, registry and preflight layer, common execution schema, statistical diagnostics, provenance controls, and independent audit.
- Long SHA-256 values were removed from the readable article and supplement and retained in machine-readable manifests.
- The manuscript-preparation AI declaration remains present but does not describe code development. The separate Methods disclosure for AI-assisted code drafting follows Elsevier's research-method disclosure policy.
...
- Both named and anonymized LaTeX source archives are flat and contain no subdirectories; the anonymized archive passes an author-identity scan.
