...
input int InpMaxSignalAgeSec = 600;
input int InpMinSecondsBetweenOrderAttempts = 900;
input int InpMaxOpenPositions = 1;
input int InpMaxSpreadPoints = 120;
input int InpDeviationPoints = 30;
input int InpStopLossPoints = 1200;
input int InpTakeProfitPoints = 1800;
input int InpMaxHoldMinutes = 720;
input string InpAllowedRules = "K06,K03,K07,S83_14,S83_13,C96_07,S105_03";
...
{
   string allowed = "," + InpAllowedRules + ",";
   string needle = "," + rid + ",";
...
void EvaluateSignal()
{
   ...
   if(signal_key == g_last_attempt_signal_key && g_last_order_attempt_time > 0 && (now - g_last_order_attempt_time) < InpMinSecondsBetweenOrderAttempts){ decision="HOLD_RETRY_COOLDOWN"; reason="same signal attempted recently"; WriteStatusKv(decision,reason,selected_rule_id,feature_date,any_signal_active,active_rule_count,signal_fresh,signal_age_sec,spread_ok,spread_points,open_positions,order_attempted); return; }
   ...
   if(ok){ g_last_success_signal_key=signal_key; decision="DEMO_BUY_SENT"; reason="demo BUY order accepted"; }
   else{ decision="DEMO_BUY_REJECTED"; reason="demo BUY order rejected by terminal/broker"; }
   WriteState();
