...
def main() -> None:
    ...
    try:
        ...
        shutil.copy(src_root / "app" / "stage81b_corrected_hard_audit_stage80_shortlist.py", root / "app")
        shutil.copy(src_root / "configs" / "stage81b_corrected_hard_audit_stage80_shortlist.json", root / "configs")
        ...
        result = subprocess.run(
            [
                sys.executable,
                str(root / "app/stage81b_corrected_hard_audit_stage80_shortlist.py"),
                "--root", str(root),
                "--config", "configs/stage81b_corrected_hard_audit_stage80_shortlist.json",
                "--out", "reports/stage81b_corrected_hard_audit_stage80_shortlist",
            ],
            text=True,
            capture_output=True,
        )
        assert result.returncode == 0, result.stderr + result.stdout
        summary = json.loads((root / "reports/stage81b_corrected_hard_audit_stage80_shortlist/stage81b_corrected_hard_audit_stage80_shortlist_summary.json").read_text())
        assert summary["status"] == "STAGE81B_COMPLETE_NO_PROMOTION"
        assert summary["candidate_count"] == 2
        assert "NO_ORDER_AUTHORIZATION_FROM_STAGE81B" in summary["hard_blocks"]
        assert (root / "reports/stage81b_corrected_hard_audit_stage80_shortlist/stage81b_corrected_hard_audit_metrics.csv").exists()
        assert (root / "reports/stage81b_corrected_hard_audit_stage80_shortlist/stage81b_selected_for_stage82.csv").exists()
        print("Stage81B tests passed")
    finally:
        shutil.rmtree(tmp)
