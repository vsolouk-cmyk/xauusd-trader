...
PROGRAM = "XAUUSD_ARTICLE1_UNIFIED_REFERENCE_RUNNER_V1_1"
...
PROTOCOL_PATH = Path("configs/article_publication/XAUUSD_ARTICLE1_EVALUATION_PROTOCOL_V1_1.json")
DEVIATION_LOG_PATH = Path("docs/article_publication/XAUUSD_ARTICLE1_PROTOCOL_DEVIATION_LOG_V1_1.json")
RUNNER_PATH = Path("tools/article_publication/article1_unified_reference_runner_v1_1.py")
FAILURE_DIR = Path("reports/article1_locked_reference_rerun_failures_v1_1")
...
class PreflightFailure(RuntimeError):
    def __init__(self, report: dict[str, Any]):
        self.report = report
        failed = [str(item["name"]) for item in report["checks"] if not item["pass"]]
        super().__init__("unified runner preflight failed: " + ",".join(failed))
...
def successor_results(
    ...
    routing = protocol["cross_feed_replication"]["input_routing"]
    amarkets_db = input_paths[routing["amarkets_h1_input_id"]]
    dukascopy_db = input_paths[routing["dukascopy_h1_input_id"]]
    events = module.load_events(input_paths["historical_event_context_sqlite"])
    candidates = module.parse_candidates(cfg)

    def feed_features(database: Path, table: str) -> pd.DataFrame:
        h1 = module.load_h1(database, table, int(cfg["minimum_m5_bar_count"]))
    ...
    am_features = feed_features(amarkets_db, protocol["cross_feed_replication"]["amarkets_h1_table"])
    dk_features = feed_features(dukascopy_db, protocol["cross_feed_replication"]["dukascopy_h1_table"])
...
def tsmom_results(
    ...
    def common_feed(database: Path, table: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    am_common_raw, am_common = common_feed(amarkets_db, protocol["cross_feed_replication"]["amarkets_h1_table"])
    dk_common_raw, dk_common = common_feed(dukascopy_db, protocol["cross_feed_replication"]["dukascopy_h1_table"])
    cross = cross_feed_comparison(
...
def sqlite_table_schema_check(
    input_paths: dict[str, Path],
    input_id: str,
    table: str,
    required_columns: Iterable[str],
) -> dict[str, Any]:
    path = input_paths.get(input_id)
    required = sorted(set(str(column) for column in required_columns))
    if path is None or not path.is_file():
        return {
            ...
            "error": "HASH_BOUND_INPUT_UNAVAILABLE",
            "pass": False,
        }
    ...
    return {
        ...
        "error": error,
        "pass": error is None and not missing,
    }
...
def outcome_blind_preflight(root: Path) -> tuple[dict[str, Any], dict[str, Path], Any]:
    ...
    cross_feed = protocol["cross_feed_replication"]
    routing = cross_feed["input_routing"]
    schema_contracts = [
        (
            routing["amarkets_h1_input_id"],
            cross_feed["amarkets_h1_table"],
        ),
        (
            routing["dukascopy_h1_input_id"],
            cross_feed["dukascopy_h1_table"],
        ),
    ]
    for input_id, table in schema_contracts:
        schema = sqlite_table_schema_check(
            input_paths,
            input_id,
            table,
            cross_feed["required_h1_columns"],
        )
        checks.append({"name": f"input_schema:{input_id}:{table}", **schema})
...
def freeze_manifest(
    ...
    manifest = {
        "program": "XAUUSD_ARTICLE1_MAIN_RUN_MANIFEST_V1_1",
        "frozen_utc": utc_now(),
...
def execute_reference(root: Path) -> tuple[dict[str, Any], Path]:
    ...
    if not preflight["pass"]:
        raise PreflightFailure(preflight)
    ...
    results: list[SpecResult] = []
    results.extend(successor_results(root, modules["successor_scan"], index, protocol, input_paths))
    ...
    output = Path.home() / "Downloads" / f"XAUUSD_ARTICLE1_REFERENCE_RERUN_V1_1_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.zip"
...
def failure_archive(root: Path, exc: Exception) -> Path:
    ...
    if isinstance(exc, PreflightFailure):
        failure["outcome_blind_preflight"] = exc.report
    json_path = root_failure / f"article1_unified_runner_failure_{stamp}.json"
    write_json(json_path, failure)
    output = Path.home() / "Downloads" / f"XAUUSD_ARTICLE1_REFERENCE_RERUN_V1_1_FAILURE_{stamp}.zip"
